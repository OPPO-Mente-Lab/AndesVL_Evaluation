...
def parse_args():
    ...
    parser.add_argument('--result_file', type=str, default=None, help='you can choose to specify the result file path')
    # Infer + Eval or Infer Only or Eval Only
...
def main():
    ...
    if args.result_file:
        assert len(args.data)==1, f"result_file {args.result_file} is specified, but multiple data files given: {args.data}"
    ...
    for _, model_name in enumerate(args.model):
        ...
        pred_root = os.environ.get("pred_root", osp.join(args.work_dir, model_name))
                ...
                if args.mode in ['all', 'eval']:
                    ...
                    if args.judge is not None:
                        ...
                        if dataset.TYPE in ['MCQ', 'Y/N'] or listinstr(['MathVerse'], dataset_name):
                            if listinstr(['WeMath','VLMBlind'], dataset_name):
                                judge_kwargs['model'] = 'gpt-4-turbo'
                            else:
                                judge_kwargs['model'] = 'chatgpt-0125'
                        elif listinstr(['MMVet', 'MathVista', 'LLaVABench', 'MMBench-Video', 'MathVision','DynaMath','MathVerse','LogicVista'],
                                    dataset_name):
                            judge_kwargs['model'] = 'gpt-4-turbo'
                        elif listinstr(['MMLongBench', 'MMDU', 'DUDE', 'DUDE_MINI', 'SLIDEVQA', 'SLIDEVQA_MINI'],
                                    dataset_name):
                            judge_kwargs['model'] = 'gpt-4o'
                        elif listinstr(['FlashMemory_STRUCT','MemoryHouse'],
                                    dataset_name):
                            judge_kwargs['model'] = 'claude-3.5-sonnet'
